...
if (obj.length > 0) {
    ...
    splitArray(obj, 3).forEach(forums => {
        forums.forEach(forum => {
            console.log(forum);
            group.append(`<input style="margin-top: 3px; ${await detectColor(forum)}" type="button" class="button" value="${forum.name}" id="${forum.id}" title="${forum.name}" name="${forum.name}" tabindex="1" onclick="buttonclick(this.id, this.name)"> `);
